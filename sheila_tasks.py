...
def _reminder_text(body: str) -> str:
    ...
    value = re.sub(r"\b(?:on\s+|for\s+)?(?:monday|tuesday|wednesday|thursday|friday|saturday|sunday)\s*,?\s+(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?\b", "", value, flags=re.IGNORECASE)
    ...
    value = re.sub(r"^\s*(?:on\s+)?(?:january|february|march|april|may|june|july|august|september|october|november|december)\s+\d{1,2}(?:st|nd|rd|th)?(?:,?\s+\d{4})?\b", "", value, flags=re.IGNORECASE)
